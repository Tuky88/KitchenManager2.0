...
public void insertar(String id, double precio,int stock,int cuantosD,int cuantosS,String tipo,String usiario,String pass)
     {
         try {
             ...
             String sql="insert into usuarios (id,getContraseña(),tipo) values('"+usiario+"','"+pass+"','"+tipo+"')";
...
public int validarPass(String Id,String pass)
        {
            int bandera=0;
            String contra=" ";
            try {
             getDatabase().MySQLConnection(getDatab(),getUser(),getContraseña());
             rs=getDatabase().getStatement().executeQuery("select * from usuarios where id='"+Id+"'");
...
public void ActualizarContraseña(String id,String contra)
{
    try {
             getDatabase().MySQLConnection(getDatab(),getUser(),getContraseña());
             String sql="update usuarios set contraseña='"+contra+"' where id='"+id+"'";
             System.out.println(sql);
            getDatabase().getStatement().execute(sql);
